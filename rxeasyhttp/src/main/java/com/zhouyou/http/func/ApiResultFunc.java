    ...
    public ApiResult<T> apply(@NonNull ResponseBody responseBody) throws Exception {
        ...
        if (type instanceof ParameterizedType) {//自定义ApiResult
            ...
            if (ApiResult.class.isAssignableFrom(cls)) {
                ...
                Log.i("kanjson","clazz"+clazz+"rawType"+rawType);
                try {
                    ...
                    ApiResult result = JSON.parseObject(json,type);
                        if (result != null) {
                            apiResult = result;
                            Log.i("kanjson","data==>"+result.getData());
                            if(apiResult.getData()==null&&clazz.equals(String.class)){
                                apiResult.setData((T) "数据为null");
...
//                apiResult.setMsg("ApiResult.class.isAssignableFrom(cls) err!!");
                apiResult.setMsg("检查数据结构是否是ApiResult");
            }
        } else {//默认Apiresult
            try {
...
////                        });
////                        HttpResult<T> httpResult= JSON.parseObject(json,HttpResult<L>.class);
////                        apiResult.setData( httpResult.getData());
//                        apiResult.setCode(0);
