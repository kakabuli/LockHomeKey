...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    private void logForRequest(Request request, Connection connection) throws IOException {
        ...
        try {
            String requestStartMessage = "请求地址：--> " + request.method() + ' ' + URLDecoder.decode(request.url().url().toString(),UTF8.name()) + ' ' + protocol;
            log(requestStartMessage);

            if (logHeaders) {
                Headers headers = request.headers();
                for (int i = 0, count = headers.size(); i < count; i++) {
//                    log("请求头：===\t" + headers.name(i) + ": " + headers.value(i));
                }
    ...
    private void bodyToString(Request request) {
        try {
            ...
            log("\t请求参数:" + URLDecoder.decode(buffer.readString(charset),UTF8.name()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
