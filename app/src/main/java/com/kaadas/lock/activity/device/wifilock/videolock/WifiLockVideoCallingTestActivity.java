...
public class WifiLockVideoCallingTestActivity extends BaseActivity<IWifiLockVideoCallingView,
        ...
        ivCalling.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){
                    if (ContextCompat.checkSelfPermission(WifiLockVideoCallingTestActivity.this, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions( WifiLockVideoCallingTestActivity.this, new String[]{Manifest.permission.RECORD_AUDIO}, 1);
                        ToastUtil.showShort("请先获取麦克风权限");
                    }else{
                        // 勾选了不再提示
                        if (! ActivityCompat.shouldShowRequestPermissionRationale
                                (WifiLockVideoCallingTestActivity.this,Manifest.permission.RECORD_AUDIO )) {
                            // ...
                            ToastUtil.showShort("请先获取麦克风权限");
                        }else{
...
//                mPresenter.release();
            ...
            case R.id.iv_setting:
                Intent settingIntent = new Intent(WifiLockVideoCallingTestActivity.this, WifiLockRealTimeActivity.class);
                settingIntent.putExtra(KeyConstants.WIFI_SN,wifiSn);
                startActivity(settingIntent);
                mPresenter.release();
                break;
            case R.id.iv_album:
                Intent intent = new Intent(WifiLockVideoCallingTestActivity.this,WifiLockVideoAlbumActivity.class);
        ...
        if(ints != null ){
            if(!b){
                ...
                runOnUiThread(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        Glide.with(WifiLockVideoCallingTestActivity.this).load(myBitmap).into(ivScreenshotBitmap);
                ...
                if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
                    ...
                    // 其次把文件插入到系统图库
                    try {
                        MediaStore.Images.Media.insertImage(WifiLockVideoCallingTestActivity.this.getContentResolver(),
                ...
                WifiLockVideoCallingTestActivity.this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(FileTool.getVideoLockPath(this,wifiLockInfo.getWifiSN()))));
    ...
    public void onStopRecordMP4CallBack(MP4Info mp4Info) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                // 最后通知图库更新
                if(mp4Info.isResult()){
                    WifiLockVideoCallingTestActivity.this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(mp4Info.getFilePath()))));
                    mPresenter.handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            llyRecord.setVisibility(View.GONE);
