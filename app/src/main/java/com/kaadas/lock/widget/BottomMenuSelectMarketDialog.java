...
public class BottomMenuSelectMarketDialog extends Dialog {

    public BottomMenuSelectMarketDialog(Context context, int themeResId) {
    ...
    public static class Builder {
		...
		private boolean goneCancel = false;
		public void goneCancel(){
			goneCancel = true;
		}


		public BottomMenuSelectMarketDialog create() {
            final BottomMenuSelectMarketDialog dialog = new BottomMenuSelectMarketDialog(p.context, shadow ? R.style.Theme_Light_NoTitle_Dialog : R.style.Theme_Light_NoTitle_NoShadow_Dialog);
            ...
            View view = LayoutInflater.from(p.context).inflate(R.layout.dialog_bottom_menu_select_market, null);
            TextView dialog_bottom_update_info= view.findViewById(R.id.dialog_bottom_update_info);
            String dialog_bottom_update_text=  p.context.getString(R.string.update_toast);
            dialog_bottom_update_text=String.format(dialog_bottom_update_text,p.versionStr);
            if(!TextUtils.isEmpty(p.versionStr)){
                dialog_bottom_update_info.setText(dialog_bottom_update_text);
            }else {
                dialog_bottom_update_info.setVisibility(View.GONE);
            }
            ...
            if (hasTitle) {
                ...
                tTitle.setBackgroundResource(R.drawable.dialog_selector_top);
            ...
            for (int i = 0; i < p.menuList.size(); i++) {
                ...
                int backgroundResId = R.drawable.dialog_selector_center;
                if (p.menuList.size() > 1) {
                    if (i == 0) {
                        if (hasTitle) {
                            backgroundResId = R.drawable.dialog_selector_center;
                        } else {
                            backgroundResId = R.drawable.dialog_selector_top;
                        }
                    } else if (i == p.menuList.size() - 1) {
                        backgroundResId = R.drawable.dialog_selector_bottom;
                    }
                } else if (p.menuList.size() == 1) {
                    backgroundResId = R.drawable.dialog_selector_singleton;
                }
