...
public class WifiVideoLockAMModePresenter<T> extends BasePresenter<IWifiVideoLockAMModeView> {

    private Disposable listenActionUpdateDisposable;
    private String wifiSN;
    private Disposable otaDisposable;
    private Disposable setAMLinstenerDisposable;
    ...
    @Override
    public void attachView(IWifiVideoLockAMModeView view) {
    ...
    public void setConnectAMMode(String wifiSN,int amMode){
...
//                    setMqttCtrl(0);
    ...
    public void setAMMode(String wifiSN,int amMode){
        if (mqttService != null && mqttService.getMqttClient() != null && mqttService.getMqttClient().isConnected()) {
            ...
            toDisposable(setAMLinstenerDisposable);
            setAMLinstenerDisposable = mqttService.mqttPublish(MqttConstant.getCallTopic(MyApplication.getInstance().getUid()),mqttMessage)
                    .filter(new Predicate<MqttData>() {
                        @Override
                        public boolean test(MqttData mqttData) throws Exception {
                            if(MqttConstant.SET_LOCK.equals(mqttData.getFunc())){
                    ...
                    .subscribe(new Consumer<MqttData>() {
                        @Override
                        public void accept(MqttData mqttData) throws Exception {
                            SetVideoLockAmModeResult setVideoLockAmModeResult = new Gson().fromJson(mqttData.getPayload(), SetVideoLockAmModeResult.class);
                            if(setVideoLockAmModeResult != null){
                                MyApplication.getInstance().getAllDevicesByMqtt(true);
                                if("200".equals(setVideoLockAmModeResult.getCode() + "")){
                                    if(isSafe()){
