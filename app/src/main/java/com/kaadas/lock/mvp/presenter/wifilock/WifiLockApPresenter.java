...
public class WifiLockApPresenter<T> extends BasePresenter<IWifiLockApView> {
    ...
    public void connectLockWifi(String homePassword, String homeSsid, String adminPasswrd, WifiUtils wifiUtils) {
        getWifiListDisposable = Observable.interval(0, 3, TimeUnit.SECONDS)//设置0延迟，每隔一秒发送一条数据
                .take(10)//设置截取前20次
                            ...
                            for (ScanResult scanResult : wifiList) {
                                ...
                                if (!TextUtils.isEmpty(sSsid)) {
                                    ...
                                    if (sSsid.startsWith("KDS_")) {
                                        ...
                                        toDisposable(getWifiListDisposable);
                                        return;
                                    }
                                }
                            }
        ...
        compositeDisposable.add(getWifiListDisposable);
    }


    public void transportData(String adminPassword, String wifiName, String wofiPassword, WifiUtils wifiUtils) {
        if (isSafe()) {
...
//                    }
                    if (isSafe()) {
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
                                mViewRef.get().readSuccess(wifiSn, randomCode, wifiName, wifiResult.func);
                            }
                        });
                    }
    ...
    public void checkNet(String wifiSN, String randomCode, String wifiName, int func) {
        checkNetDisposable = Observable.interval(0, 1, TimeUnit.SECONDS)//设置0延迟，每隔一秒发送一条数据
                .take(10)//设置截取前20次
                .compose(RxjavaHelper.observeOnMainThread())
                .subscribe(new Consumer<Long>() {
                    @Override
        ...
        compositeDisposable.add(checkNetDisposable);
    }
