...
import com.kaadas.lock.publiclibrary.ota.ble.p6.CommonUtils.ConvertUtils;
...
public class OTAResponseReceiver_v1 extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (BluetoothLeService.ACTION_OTA_DATA_AVAILABLE_V1.equals(action)) {
            ...
            } else if (stringSharedPreference.equalsIgnoreCase("" + BootLoaderCommands_v1.SEND_DATA)) {
                parseSendDataCmdResponse(respBytes);
            } else if (stringSharedPreference.equalsIgnoreCase("" + BootLoaderCommands_v1.PROGRAM_DATA)) {
                parseProgramDataCmdResponse(respBytes);
            } else if (stringSharedPreference.equalsIgnoreCase("" + BootLoaderCommands_v1.VERIFY_APP)) {
                parseVerifyAppResponse(respBytes);
            } else if (stringSharedPreference.equalsIgnoreCase("" + BootLoaderCommands_v1.EXIT_BOOTLOADER)) {
                parseExitBootloaderCmdResponse(respBytes);
            } else {
                Log.e("中转广播接受者","Unknown PREF_BOOTLOADER_STATE: " + stringSharedPreference);
            }
        }
    }
    ...
    private void broadCastErrors(int errorKey) {
        switch (errorKey) {
            ...
            case CASE_ERR_CHECKSUM:
                Logger.e("CYRET_ERR_CHECKSUM    校验和错误");
