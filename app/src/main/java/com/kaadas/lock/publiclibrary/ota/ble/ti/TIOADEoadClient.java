...
import com.kaadas.lock.publiclibrary.ota.ble.ti.BluetoothLEController.BluetoothLEDevice;
...
public class TIOADEoadClient {
    ...
    public void start(String filename) {
        ...
        byte[] fileImageInfo = fileReader.imageHeader.TIOADEoadImageIdentificationValue;  //文件读取的header
        byte[] oadClientImageInfo = TIOADEoadDefinitions.oadImageInfoFromChipType(TIOADEoadDeviceID);


        Log.e(TAG, "fileImageInfo   " + Rsa.bytesToHexString(fileImageInfo) + " oadClientImageInfo  " + Rsa.bytesToHexString(oadClientImageInfo));
        Log.e(TAG, "fileImageInfo   " + new String(fileImageInfo) + " oadClientImageInfo  " + new String (oadClientImageInfo));
        for (int i = 0; i < 8; i++) {
            if (fileImageInfo[i] != oadClientImageInfo[i]) imageCanBeProgrammed = false;
        }

        if (TIOADEoadDeviceID[0] == 0x08){
            imageCanBeProgrammed = true;
        }
    ...
    public void handleOADControlPointMessage(final byte[] response) {
        switch (response[0]) {
            case TIOADEoadDefinitions.TI_OAD_CONTROL_POINT_CMD_DEVICE_TYPE_CMD:
                ...
                Log.e(TAG, "获取到  ID  Device ID: " + TIOADEoadDefinitions.oadChipTypePrettyPrint(TIOADEoadDeviceID));
