...
//        com.kaidishi.lock.MainActivity.logList.add(0, getSimpleDate() + " " + log);
        Log.e("denganzhi1","点击通知栏:"+message);
        if (!TextUtils.isEmpty(message.getTopic())) {
            mTopic = message.getTopic();
        } else if (!TextUtils.isEmpty(message.getAlias())) {
            mAlias = message.getAlias();
...
//        DemoApplication.getHandler().sendMessage(msg);
   ...
   //用来接收客户端向服务器发送命令消息后返回的响应，onReceiveRegisterResult用来接受客户端向服务器发送注册命令消息后返回的响应
    @Override
    public void onCommandResult(Context context, MiPushCommandMessage message) {
//        Log.v(DemoApplication.TAG,
//                "onCommandResult is called. " + message.toString());
        Log.e("denganzhi1","onCommandResult:"+message);
            ...
            if (message.getResultCode() == ErrorCode.SUCCESS) {
             ...
             //   log = context.getString(R.string.register_success);
              ...
              //  log = context.getString(R.string.register_fail);
                Log.e("denganzhi1","XiaoMI_获取Regester失败");
            }
...
//                "onReceiveRegisterResult is called. " + message.toString());
        Log.e("denganzhi1","onReceiveRegisterResult:"+message);
        String command = message.getCommand();
        List<String> arguments = message.getCommandArguments();
        String cmdArg1 = ((arguments != null && arguments.size() > 0) ? arguments.get(0) : null);
        String log;
...
//            if (message.getResultCode() == ErrorCode.SUCCESS) {
//                mRegId = cmdArg1;
//                Log.e(DemoApplication.TAG,"onReceiveRegisterResult==>"+mRegId);
//                log = context.getString(R.string.register_success);
//            } else {
//                log = context.getString(R.string.register_fail);
//            }
...
//            intent.setComponent(new ComponentName(context.getPackageName(), com.kaidishi.lock.PermissionActivity.class.getCanonicalName()));
