        ...
        if (data != null && data.getExtras() != null) {
            ...
            if (directPhoto) {
                if (!(checkPermission(Manifest.permission.CAMERA))) {
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.CAMERA,Manifest.permission.WRITE_EXTERNAL_STORAGE}, ImageGridActivity.REQUEST_PERMISSION_CAMERA);
                } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_PERMISSION_STORAGE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                showToast(getString(R.string.not_allow_sd_warring));
            }
        } else if (requestCode == REQUEST_PERMISSION_CAMERA) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                imagePicker.takePicture(this, ImagePicker.REQUEST_CODE_TAKE);
            } else {
                showToast(getString(R.string.not_allow_camera_warring
                ));
            }
        }
    }
...
//                    mImageGridAdapter.refreshData(imageFolder.images);
    ...
    public void onImagesLoaded(List<ImageFolder> imageFolders) {
        Log.e("数据加载完成  数量是  ",""+imageFolders.size());
...
//        mRecyclerAdapter.notifyItemChanged(position + (imagePicker.isShowCamera() ? 1 : 0));// 17/4/24  fix the position while click right bottom preview button
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.e("跳转返回了数据  ", "data是否为空  " + (data==null));
        if (data != null && data.getExtras() != null) {
