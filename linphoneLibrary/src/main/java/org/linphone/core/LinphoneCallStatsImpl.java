...
class LinphoneCallStatsImpl implements LinphoneCallStats {
	...
	private native float getTotalLossRate(long nativeStatsPtr);
	protected LinphoneCallStatsImpl(long nativeStatsPtr) {
		...
		localLossRate = getLocalLossRate(nativeStatsPtr);
		totalLossRate=getTotalLossRate(nativeStatsPtr);
	}
