...
public class MultiImageView extends LinearLayout {
	...
	private int pxImagePadding = SizeUtils.dp2px(5);// 图片间的间距
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		...
		if(isMultiImage){
			...
			imageView.setLayoutParams(moreParaColumnFirst);//原来是这个属性onePicPara,为了显示小图,才换成moreParaColumnFirst
		}

		imageView.setTag(R.string.zone_img_position,position);
		imageView.setId(url.hashCode());
		imageView.setOnClickListener(mImageViewOnClickListener);
		GlidUtils.loadImageNormal(getContext(), StringUtil.getImageUrl(url),imageView,R.drawable.image_loading);
		return imageView;
	}
