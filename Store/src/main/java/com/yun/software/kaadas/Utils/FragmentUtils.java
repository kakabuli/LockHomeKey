...
public class FragmentUtils {
    ...
    public static Fragment switchContent(FragmentManager fragmentManager, Fragment currentFragment, Fragment toShowFragment, int viewId, long itemId, boolean isInnerReplace) {
        ...
        if (currentFragment == null) {
            transaction.add(viewId, toShowFragment, FragmentUtils.makeFragmentName(viewId, itemId)).commitAllowingStateLoss();
//            LogUtil.t(TAG).d("currentFragment is null,just add new one");
            LogUtils.iTag(TAG,"目前是新的，你刚好添加第一个");
            return toShowFragment;
        }
        //如果当前等于要显示的，直接返回，其实并不会触发，因为点击相同位置，BottomNavigationBar 并不会回调点击事件
        if (currentFragment == toShowFragment) {
            LogUtils.iTag(TAG,"目前相同");
            return toShowFragment;
        }
        ...
        if (fragment == null) {
            ...
            transaction.hide(currentFragment).add(viewId, toShowFragment, FragmentUtils.makeFragmentName(viewId, itemId)).commitAllowingStateLoss();
            LogUtils.iTag(TAG,"old fragment is null,just hide and add new one");
            return toShowFragment;
        } else {
            ...
            if (isInnerReplace) {
                ...
                //再add新的
                transaction.add(viewId, toShowFragment, FragmentUtils.makeFragmentName(viewId, itemId)).commitAllowingStateLoss();
                LogUtils.iTag(TAG,"isInnerReplace is true ,remove old and add new");
            } else {
                transaction.hide(currentFragment).show(fragment).commit();
                LogUtils.iTag(TAG,"isInnerReplace is false ,just hide and show");
            }
            return toShowFragment;
        }
    }
