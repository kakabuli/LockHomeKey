 ...
 * on 2019/3/15
 */
public class AAAAListActivity extends BaseActivity {
    ...
    protected void initViewsAndEvents() {
        tvTitle.setText("测试");
        for (int i=0; i<3; i++){
            listBeans.add("text"+i);
        }

        listAdapter=new AAAAListAdapter(listBeans);
...
//        map.put("page",page);
//        HttpManager.getInstance().post(mContext, ApiConstants.HOME_FREEPAGE, map, new OnIResponseListener() {
//            @Override
...
////                Gson gson = new Gson();
////                BaseBody<HomeListBean> baseBody = gson.fromJson(result,new TypeToken<BaseBody<HomeListBean>>(){}.getType());
////                listBeans.addAll(baseBody.getRows());
////                total = baseBody.total;
////                mFreeExperienceListAdapter.notifyDataSetChanged();
