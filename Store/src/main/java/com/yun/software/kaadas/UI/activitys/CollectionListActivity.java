...
import com.yun.software.kaadas.UI.adapter.CollectionAdapter;
...
public class CollectionListActivity extends BaseActivity {
    ...
    protected void initViewsAndEvents() {
        ...
        listAdapter = new CollectionAdapter(this,listBeans);
...
//        recyclerView.setLayoutManager(new GridLayoutManager(mContext, 2));
        ...
        listAdapter.setOnClickListener(new CollectionAdapter.OnClickListener() {
            @Override
            public void onClick(int position) {
                if (editStatus){
                    isCheckAll = true;
                    for (MyCollection bean:listBeans){
                        if (!bean.isChecked()){
                            isCheckAll = false;
                            break;
                        }
                    ...
                    MyCollection collection = listBeans.get(position);
                    Bundle bundle = new Bundle();
                    bundle.putInt("type",ShopDetailsNewActivity.TYPE_NORMAL);
                    bundle.putString("id",collection.getId());
    ...
    private void deleteData() {
        ...
        HttpManager.getInstance().post(mContext, ApiConstants.COLLECTAPP_CANCELCOLLECTBYIDS, map, new OnIResponseListener() {
            @Override
            public void onSucceed(String result) {

                Iterator<MyCollection> it = listBeans.iterator();
                while(it.hasNext()){
                ...
                if (listBeans.size() == 0){
                    ...
                    toggleShowEmptyImage(true, R.drawable.collect_missing_pages, new View.OnClickListener() {
                        @Override
    ...
    private void getData() {
        ...
        map.put("page",page);
        HttpManager.getInstance().post(mContext, ApiConstants.COLLECTAPP_PAGE, map, new OnIResponseListener() {
            @Override
            public void onSucceed(String result) {
                toggleRestore();
                Gson gson = new Gson();
                BaseBody<MyCollection> baseBody = gson.fromJson(result,new TypeToken<BaseBody<MyCollection>>(){}.getType());
